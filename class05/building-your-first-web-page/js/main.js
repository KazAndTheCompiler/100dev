...
// Wait for glMatrix to be available
function start() {
	...
	(function() {
		const vec3 = glMatrix.vec3 || window.vec3;
		const mat4 = glMatrix.mat4 || window.mat4;
	...
	class Renderer {
		...
		constructor(scene){
			...
			this.tPos1 = vec3.create();
			this.tPos2 = vec3.create();
			this.tPos3 = vec3.create();
		...
		drawLine(start, end, color){
			...
			if(tPos2[2] < 0) return;
			canvasCtx.beginPath();
			canvasCtx.moveTo(tPos1[0], tPos1[1]);
			canvasCtx.lineTo(tPos2[0], tPos2[1]);
			canvasCtx.strokeStyle = color;
			canvasCtx.stroke();
		}
